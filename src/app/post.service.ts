...
export class PostService {
 ...
 //   'Authorization': `Bearer ${token}`
  // })})
   return this.client.post('https://mighty-cove-30860.herokuapp.com/create/post', command, {headers: new HttpHeaders({
     'Content-Type': 'application/json',
     'Authorization': `Bearer ${token}`
    })})
  }
 ...
 //   return this.client.post('http://localhost:8080/add/comment', command, {headers: new HttpHeaders({
  ...
  //   'Authorization': 'Bearer ${token}'
  //  })})
    return this.client.post('https://mighty-cove-30860.herokuapp.com/add/comment', command, {headers: new HttpHeaders({
     'Content-Type': 'application/json',
     'Authorization': 'Bearer ${token}'
    })})
  }
  ...
  login(command:any): Observable<Token>{
   // return this.client.post<Token>('http://localhost:8080/auth/login', command, this.httOptions)
    return this.client.post<Token>('https://mighty-cove-30860.herokuapp.com/auth/login', command, this.httOptions)
